...
class WebSocketService {
  ...
  connect(params?: { userId?: string; doctorId?: string; userName?: string }) {
    try {
      ...
      // 尝试多种连接地址和端口
      let wsHost;
      let wsPort = 8082; // 使用8082端口避免与Metro bundler冲突
      ...
      const url = `ws://${wsHost}:${wsPort}/chat?userId=${this.connectionParams.userId}&doctorId=${this.connectionParams.doctorId}&userName=${encodeURIComponent(this.connectionParams.userName)}`;
