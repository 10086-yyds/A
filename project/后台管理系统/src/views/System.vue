          ...
          <div class="permission-section">
            <div class="permission-header">
              ...
              <el-button
                type="text"
                size="small"
                @click="showPermissionPreview = !showPermissionPreview"
              >
            ...
            <el-checkbox-group v-model="roleForm.permissions" class="permission-group">
              <div v-for="option in permissionOptions" :key="option.key" class="permission-item">
                ...
                <!-- 权限预览 -->
                <div
                  v-if="showPermissionPreview && roleForm.permissions.includes(option.key)"
                  class="permission-preview"
                >
                  <div class="preview-title">可访问的路由：</div>
                  <div class="route-list">
                    <el-tag
                      v-for="route in option.routes"
                      :key="route"
                      size="small"
                      type="info"
                      class="route-tag"
            ...
            <div v-if="roleForm.permissions.length > 0" class="permission-summary">
              <el-alert
                :title="`已选择 ${roleForm.permissions.length} 个权限模块`"
                type="info"
                :closable="false"
                show-icon
              >
                <template #default>
                  ...
                  <div class="selected-permissions">
                    <el-tag
                      v-for="perm in roleForm.permissions"
                      :key="perm"
                      size="small"
                      type="success"
                      class="selected-tag"
        ...
        <el-form-item label="选择账号" prop="userId">
          <el-select v-model="deleteForm.userId" placeholder="请选择要删除的账号" filterable>
            <el-option
              ...
              :key="position.id"
              :label="`${position.username} (${position.realName})`"
              :value="position.id"
            />
          </el-select>
        </el-form-item>
...
const createUser = async () => {
  ...
  try {
    ...
    console.log('选择的角色ID:', userForm.roleId) // 调试用

    const response = await http.post('/lz/add', userData)
    console.log('后端响应:', response) // 调试用
    // 如果到这里说明请求成功（失败会被axios拦截器处理）
